...
// on the CYD 2.8" esp32 2432S028(2USB) version.
// Make sure you have the correct Tft_espi User Setup correctly set for your board.
// All thanks to the original Author Above who created this fantastic code, which when ported allows such a simple device to be used
// with no soldering skills or breadboards etc.
// Dont forget to set up your wifi credentials and the timezone information that you require
...
#define WIFI_SSID          "SSID"            //   your credentials
#define WIFI_PWD           "SSID Password"  //    
#define NTP_SERVER         "pool.ntp.org"          // time.nist.gov, pool.ntp.org, etc    
    
#define TZ_RULE            "GMT0BST,M3.5.0/1:00:00,M10.5.0/1:00:00" // Set this to your Local timezone(uk)
#define TZ_RULE1            "CST6CDT,M3.2.0/2:00:00,M11.1.0/2:00:00"//////////Set lower display rule 2nd timezone CoolMD

//#define TZ_RULE            "EST5EDT,M3.2.0/2:00:00,M11.1.0/2:00:00"

#define SCREEN_ORIENTATION       3                 // screen portrait mode:  use 1 or 3
#define LED_PIN                 17                 // built-in LED is on GPIO pins 4 red, 16 green, 17 blue 
...
#define LOCAL_FORMAT_12HR     true                 // local time format true 12hr "11:34" false 24hr "23:34"
#define UTC_FORMAT_12HR      true                  // 2nd timezone time format 12 true hr "11:34" vs  false 24hr "23:34"
#define DISPLAY_AMPM          true                 // if true, show A or P 
...
void showTimeZone (int x, int y) {
  ...
  if (!useLocalTime) 
    ...
    tft.drawString(myTZ.getTimezoneName(),x,y,f);  //display 2nd Timezone in lower window location Coolmd
  else 
...
void updateDisplay() {
  t = myTZ.now();                                  // check latest time////// for new Timezone Coolmd
  if (t!=oldT) {                                   // are we in a new second yet?
    ...
    useLocalTime = false;                          // use 2ND timezone/////////////////////////////////////Coolmd
...
void printTime() {                                 // print time to serial port
  ...
  Serial.print("TIME: ");
  if (PRINTED_TIME==1)
    Serial.println(myTZ.dateTime(TIME_FORMAT));         // option 1: print 2nd timezone time////////////////////////
  else 
...
void setup() {
  ...
  myTZ.setPosix(TZ_RULE1);                         //estab. 2nd Lower TZ by rule defined above Coolmd
